...
二开版本号采用四段格式：前三段跟随上游版本，第四段表示自定义修订。例如上游 `0.1.126` 的第一次 Kiro 二开版本为 `0.1.126.1`，对应 GitHub tag 为 `v0.1.126.1`。
...
`catchyan/sub2api` 的 main 分支内置 Kiro 平台，添加账号时选择 **Kiro**。只支持 `kiro-gateway` 已验证的三种凭据来源：

- JSON 导入：Kiro Desktop JSON、AWS SSO OIDC JSON，以及带 `clientIdHash` 的企业凭据。
- SQLite 导入：kiro-cli / amazon-q 的 SQLite，导入 `auth_kv` 与 `state` 中的已知 token 和 device registration 键。
- Refresh Token：仅支持 Kiro Desktop refresh token，不提供浏览器 OAuth 或通用 OAuth 登录。

导入会把凭据规范化保存到 sub2api 数据库；不会保存本地文件路径，不会在运行时读取原始 JSON/SQLite，也不会写回 SQLite。企业 `clientIdHash` 场景需要同时上传 companion SSO cache JSON，或手动填写 `client_id` / `client_secret`。
